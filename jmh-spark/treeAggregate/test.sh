...
OUTPUT_DIR=/home/hibench-output/tuning_url/url_local
...
#	./bin/workloads/ml/lr/prepare/prepare.sh | tee -a $OUTPUT_DIR/lr/experiment_log.txt
  ...
  do
	...
	export MY_SPARK_DRIVER_MEMORY=$j
    ...
    echo -e "\e[95m===============================================" | tee -a $OUTPUT_DIR/lr/experiment_log.txt
    echo "Starting Spark LR example, $i features, $j..." | tee -a $OUTPUT_DIR/lr/experiment_log.txt
    echo -e "================================================\e[97m" | tee -a $OUTPUT_DIR/lr/experiment_log.txt
    date | tee -a $OUTPUT_DIR/lr/experiment_log.txt
	  $WORK_DIR/spark/bin/spark-submit --properties-file $PROJ_DIR/myspark.conf --driver-java-options "-XX:+PrintFlagsFinal" --class com.intel.hibench.sparkbench.ml.LogisticRegression --master name /CMC/kmiecseb/HiBench/sparkbench/assembly/target/sparkbench-assembly-6.1-SNAPSHOT-dist.jar hdfs://localhost:9000/HiBench/LR/Input | grep Heap
    date | tee -a $OUTPUT_DIR/lr/experiment_log.txt
    echo -e "\e[95m===============================================" | tee -a $OUTPUT_DIR/lr/experiment_log.txt
