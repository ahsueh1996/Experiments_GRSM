    ...
    numClasses match {
	  ...
	  var temp2 = 0.0
	    ...
	    // add it in and subtrace exp(0), or equivilantly 1.0
            temp += math.exp(-maxMargin) - 1.0
	    temp2 = if (ilabel > 0) math.log1p(temp) - marginY + maxMargin else math.log1p(temp) + maxMargin
          } else {
            for (i <- 0 until numClasses - 1) {
              temp += math.exp(margins(i))
            }
	    temp2 = if (ilabel > 0) math.log1p(temp) - marginY else math.log1p(temp)
          }
          (temp,temp2)
        }
